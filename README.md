...
容器內的 `/var/compare` 與 `/app/log` 分別綁定到主機專案下的 `var/compare` 與 `log`，應用程式會自動建立需要的資料夾。

- `log/`：儲存每次 LLM 分析產生的獨立 HTML 報告，預設保留 14 天（兩週）後由背景任務自動刪除
- `jobs`：每小時檢查，傳統比對任務預設保留 24 小時後刪除
- `llm_debug`：已預設關閉儲存，不再向磁碟寫入除錯截圖與 prompt
- 清理於 API 啟動時即刻執行一次，隨後定時巡檢清理
